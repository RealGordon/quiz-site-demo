...
$.getJSON('/confessions/comments/'+blg+'?q=userquestions',function(r,s,x){
if (s == "success"){
$('#comments-container').comments({
  getComments: function(success, error) {
	dispComments=success;
...
function likef(commentJSON, success, error) {
 ...
 type:'POST',
 dataType:'json',
 url:'/confessions/comments/likes/'+blg+'?q=userquestions',
...
localforage.setItem(c, v).then(function (value) {
 ...
 //checkco=commentJSON;
     $.ajax({
	 type:'POST',
	 url:'/confessions/comments/'+blg+'?q=userquestions', 
	 data:JSON.stringify(commentJSON),
	 dataType:'json',
	 contentType: 'application/json',
     success:function(r) {
