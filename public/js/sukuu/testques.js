function qdisplay(){
    ...
    qal.innerHTML=String.fromCharCode(OpCode++)+'.  '+ovs[i];
    ov.appendChild(qa);
    ov.appendChild(qal);
    elta.appendChild(ov);
...
function testTimer(th){
    // Set the date we're counting down to
//var countDownDate = new Date("Jan 5, 2022 15:37:25").getTime();

var seconds=0,sec=0,minutes=0,hours=0;
//time_category="sec";if (time_category=="min") duration=minutes;
  //else if (time_category=="hrs") duration=hours;//else duration=seconds;
if (max_time!=0) {
...
var x = setInterval(function() {
  ...
  seconds+=1;
  sec= seconds % 60 ;
   ((seconds>60 && sec==0) ? (minutes+=1,(
   ((minutes==60) ? (minutes=0, true ): 0)
    ? hours+=1 : 0))
   : 0);
  ...
  document.getElementById("test_timer").innerText =  hours + "h "
  + minutes + "m " + sec + "s ";
  
  // If the count down is finished, write some text
  if (seconds >= max_time) {
    clearInterval(x);
    document.getElementById("test_timer").innerText= "TEST ENDED";
