...
class User:
    ...
    def __init__(self,data=None,doc=None):
        ...
        if doc:
            ...
            for doc in s:
                ...
                self.updateUserSex(data.get('sex'))                      
    ...
    def updateUserSex(sex="M",m=None):
        ...
        if sex=="M" or sex=="F":
            ...
            doc_ref.set({'users':
                {render_field_path(str(dt.date.today().year)):
                {sex:firestore.Increment(1)}
                }},
                    merge=True)
    ...
    def save_answer(self,subject,year,sec,n_a,num):
        """        (str,str,str,dict,int)
        (category,year,section,new answer,num of questions)"""
        ...
        for k,v in n_a.items():
            if v:
                answers[int(k)]=v
        self.doc_ref.update(
            {render_field_path([subject,year,sec,'answers']):answers})
    @classmethod
